...
const assignKeyWorkers = (caseFiles) => (keyWorkers) => {
  for (var casefile_id in caseFiles) {
    assignKeyWorker(keyWorkers, caseFiles[casefile_id]);
  }
...
const generateKeyWorkerAssignment = (caseFiles) =>
  KeyWorkerServiceConnection.listKeyWorkers().then(assignKeyWorkers(caseFiles));
...
module.exports.listCaseFiles = () =>
  getCaseFiles().then(objToList).then(inspect);
